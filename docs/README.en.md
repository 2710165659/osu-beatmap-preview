...
| `osu-beatmap-preview-wasm` | Connects the core session and renderer to a browser WebGPU canvas, and exports `beatmapInfo` for summarising a beatmap from its `.osu` bytes (part of the web package) |
| `osu-beatmap-preview-web` | Vue static site and Node.js download backend: downloads and caches `.osu`/`.osz` and serves audio and background to the browser (release artifact, outside the Cargo workspace) |
