...
use crate::infrastructure::media::image;
...
fn gif_judgement_line_offset(row_height: i64) -> i64 {
    pyround(
        crate::render::modes::taiko::constants::REFERENCE_JUDGEMENT_X * row_height as f64
            / crate::render::modes::taiko::constants::TAIKO_BASE_HEIGHT,
    )
}
...
#[derive(Debug, Clone)]
pub(crate) struct AnimationLayout {
...
fn render_taiko_segment_gif(
    ...
    image::save_animated_gif_streamed(
        frame_count,
        render,
        output_path,
        frame_duration_ms,
...
pub(crate) fn compute_time_range() -> f64 {
    let in_length = crate::render::modes::taiko::constants::ASPECT_RATIO
        * crate::render::modes::taiko::constants::STABLE_GAMEFIELD_HEIGHT
        - crate::render::modes::taiko::constants::STABLE_HIT_LOCATION;
    in_length / 100.0 * 1000.0 / crate::render::modes::taiko::constants::VELOCITY_MULTIPLIER
}
...
pub(crate) fn build_animation_layout(time_range: f64) -> AnimationLayout {
...
pub(crate) fn build_animation_layout_with_segments_and_format(
    time_range: f64,
    segment_count: usize,
    ...
    let left_panel_width =
        pyround(row_height as f64 * crate::render::modes::taiko::constants::DRUM_PANEL_WIDTH_RATIO);
    let right_panel_width = geometry.playfield.width - left_panel_width;
    let inner_padding = crate::render::geometry::scale_px(
        crate::render::modes::taiko::constants::ROW_INNER_PADDING_X as f64,
        render_scale,
    );
    ...
    let (image_width, image_height, playfield_left, first_row_top, row_stride) =
        if output_format == crate::render::geometry::OutputFormat::Gif {
            let config = &crate::infrastructure::config::current().render.taiko.gif;
    ...
    let normal_note_diameter =
        pyround(row_height as f64 * crate::render::modes::taiko::constants::NORMAL_NOTE_SIZE_RATIO);
    let big_note_diameter = pyround(
        normal_note_diameter as f64 * crate::render::modes::taiko::constants::BIG_NOTE_SCALE,
    );
    let (track_background_color, track_edge_color, track_accent_color, judgement_line_color) =
        match output_format {
            ...
            crate::render::geometry::OutputFormat::Mp4 => {
                let config = &crate::infrastructure::config::current().render.taiko.mp4;
                (
                    config.style.TRACK_BACKGROUND_COLOR,
                    config.style.TRACK_EDGE_COLOR,
                    config.style.TRACK_ACCENT_COLOR,
                    crate::render::modes::taiko::constants::MP4_JUDGEMENT_LINE_COLOR,
                )
            }
...
fn gif_row_top(row_index: i64, layout: &AnimationLayout) -> i64 {
    layout.first_row_top + row_index * layout.row_stride
}

fn gif_row_center_y(row_index: i64, layout: &AnimationLayout) -> i64 {
    gif_row_top(row_index, layout) + layout.row_height / 2
}

fn judgement_line_x(layout: &AnimationLayout) -> i64 {
...
pub(crate) fn draw_row_background(image: &mut Img, layout: &AnimationLayout, row_index: i64) {
    let row_top = gif_row_top(row_index, layout);
...
fn object_x(note_time: f64, snapshot_time: f64, multiplier: f64, layout: &AnimationLayout) -> i64 {
...
fn draw_hit_object(
    ...
    if base.hit_type & SWELL_FLAG != 0 {
        draw_span_object(
            ...
            cache,
            true,
            crate::render::modes::taiko::constants::SWELL_COLOR,
            true,
        );
        return;
    }
    if base.hit_type & DRUMROLL_FLAG != 0 {
        ...
        draw_span_object(
            ...
            cache,
            is_big_roll,
            crate::render::modes::taiko::constants::ROLL_COLOR,
            false,
        );
        return;
    }
...
fn draw_circle_object(
    ...
    let color = if is_rim {
        crate::render::modes::taiko::constants::RIM_NOTE_COLOR
    } else {
        crate::render::modes::taiko::constants::CENTRE_NOTE_COLOR
    };
...
fn draw_span_object(
    ...
    let body_ratio = if use_large_geometry {
        crate::render::modes::taiko::constants::SWELL_BODY_HEIGHT_RATIO
    } else {
        crate::render::modes::taiko::constants::SPAN_BODY_HEIGHT_RATIO
    };
