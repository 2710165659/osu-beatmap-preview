...
pub(crate) fn draw_track_background(
    image: &mut Img,
    x: i64,
    y: i64,
    w: i64,
...
pub(crate) fn build_note_disc(
    ...
    let r = c;
    let ring =
        (d as f64 * crate::render::modes::taiko::constants::NOTE_RING_THICKNESS_RATIO).max(1.0);
    let fill: [u8; 4] = [color[0], color[1], color[2], 255];
    img.fill_circle_aa(
        c,
        c,
        r,
        crate::render::modes::taiko::constants::NOTE_EDGE_COLOR,
    );
    ...
    img.fill_circle_aa(
        ...
        c,
        r - edge_width,
        crate::render::modes::taiko::constants::NOTE_RING_COLOR,
    );
...
pub(crate) fn build_drum_roll_tick_sprite(diameter: i64, alpha: f64) -> Img {
    ...
    let color = crate::render::modes::taiko::constants::DRUM_ROLL_TICK_COLOR;
