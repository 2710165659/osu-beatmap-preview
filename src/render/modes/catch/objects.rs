...
pub(crate) fn banana_color(seed: i64) -> [u8; 3] {
    crate::render::modes::catch::constants::BANANA_COLORS[stateless_next_int(3, seed, 0) as usize]
}
...
pub(crate) fn build_catch_render_objects(
    ...
    let mut rng = LegacyRandom::new(crate::render::modes::catch::constants::RNG_SEED as u32);
    ...
    let skin_combo_colors = &crate::infrastructure::config::current().skin.COMBO_COLORS;
    let combo_colors: &[[u8; 3]] = if !beatmap.combo_colors.is_empty() {
        &beatmap.combo_colors
    } else if !skin_combo_colors.is_empty() {
        skin_combo_colors
    } else {
        &crate::render::modes::catch::constants::LAZER_COMBO_COLORS
    };
...
fn build_banana_shower_objects(
    ...
    while current_time <= end_time as f32 {
        let x = rng.next_double() * crate::render::modes::catch::constants::PLAYFIELD_WIDTH;
        rng.next();
        rng.next();
        rng.next();
        ...
        out.push(RenderObject {
            ...
            scale_factor: crate::render::modes::catch::constants::BANANA_SCALE,
        ...
        match event.event_type {
            EventType::Tick => {
                ...
                nested_objects.push(RenderObject {
                    ...
                    start_time: st,
                    color: combo_color,
                    scale_factor: crate::render::modes::catch::constants::DROPLET_SCALE,
        ...
        match obj.object_type {
            ObjType::TinyDroplet => {
                ...
                obj.x = if shifted_x.is_nan() {
                    ...
                    shifted_x.clamp(0.0, crate::render::modes::catch::constants::PLAYFIELD_WIDTH)
                };
            }
            ObjType::Droplet => {
                rng.next();
...
fn build_tiny_droplets_between(
    ...
    while offset < since_last_event as f64 - 0.001 {
        ...
        out.push(RenderObject {
            ...
            start_time: rhe(time),
            color: combo_color,
            scale_factor: crate::render::modes::catch::constants::TINY_DROPLET_SCALE,
...
fn apply_random_offset(position: f64, max_offset: f64, rng: &mut LegacyRandom) -> f64 {
    ...
    (position + offset).clamp(0.0, crate::render::modes::catch::constants::PLAYFIELD_WIDTH)
}

fn apply_offset(position: f64, amount: f64) -> f64 {
    (position + amount).min(crate::render::modes::catch::constants::PLAYFIELD_WIDTH)
}

fn apply_hyper_dash(render_objects: &mut [RenderObject], circle_size: f64) {
    let catcher_width =
        crate::render::modes::catch::constants::CATCHER_BASE_SIZE * circle_scale(circle_size);
