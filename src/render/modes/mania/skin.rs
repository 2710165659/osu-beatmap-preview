...
pub(crate) fn load_mania_skin_config(
    ...
    macro_rules! block {
        ($name:ident) => {
            from_config(
                &crate::infrastructure::config::current().skin.MANIA.$name,
                keys as usize,
                fallback,
            )
        };
...
fn fallback_for_format(output_format: crate::render::geometry::OutputFormat) -> SkinFallback {
    match output_format {
        crate::render::geometry::OutputFormat::Png => SkinFallback {
            hit_position: crate::infrastructure::config::current()
                .render
                .mania
                .png
                .sizing
                .HIT_TARGET_FROM_BOTTOM as f64,
            lane_width: crate::infrastructure::config::current()
                .render
                .mania
                .png
                .sizing
...
macro_rules! impl_mania_skin_block {
    ($($name:ident),+ $(,)?) => {
        $(
            impl ManiaSkinBlock for crate::infrastructure::config::$name {
