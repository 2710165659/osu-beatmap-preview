...
pub(crate) fn render_slider_body_layer(
    ...
    let scale = crate::infrastructure::config::current()
        .render
        .standard
        .png
        .style
    ...
    let inner_width = py_round(
        width as f64
            * (1.0 - crate::render::modes::standard::constants::ARGON_SLIDER_BORDER_PORTION),
    )
    .max(1);
    let body_alpha = py_round(
        alpha_byte as f64 * crate::render::modes::standard::constants::ARGON_SLIDER_BODY_ALPHA,
    )
    .clamp(0, 255) as u8;
    ...
    if traceable {
        ...
        // 用背景色擦除中心填充（游戏区域为黑色）。
        let bg = crate::infrastructure::config::current()
            .render
            .standard
            .png
            .style
...
pub(crate) fn slider_snaked_range(
    ...
    if snapshot_time < hit_object.start_time {
        if crate::render::modes::standard::constants::SNAKING_IN_SLIDERS {
    ...
    if span >= span_count - 1 && crate::render::modes::standard::constants::SNAKING_OUT_SLIDERS {
        if span % 2 == 1 {
            end = span_progress;
        } else {
            start = span_progress;
...
fn build_slider_ball(diameter: i64, circle_diameter: i64, color: [u8; 3]) -> Img {
    ...
    let border = 2.5
        ...
        * crate::render::modes::standard::constants::ARGON_BORDER_RATIO;
