...
use crate::realtime::{
    CancellationToken, FrameRequest, OffscreenConfig, RealtimeError, RealtimeSession,
    RenderedFrame, RendererInfo, RgbaFrame,
};
...
impl OffscreenRenderer {
    pub async fn new(config: OffscreenConfig) -> Result<Self, RealtimeError> {
        validate_config(config)?;
        let backends = wgpu::Backends::from_env().unwrap_or_else(wgpu::Backends::all);
        if backends.is_empty() {
            return Err(RealtimeError::GpuUnavailable(
        ...
        let (device, queue) = adapter
            ...
            .await
            .map_err(|error| RealtimeError::Device(error.to_string()))?;
    ...
    pub async fn render(&mut self, scene: &FrameScene) -> Result<RgbaFrame, RealtimeError> {
    ...
    fn submit_scene(
        ...
        scene: &FrameScene,
        buffer_index: usize,
    ) -> Result<PendingReadback, RealtimeError> {
    ...
    fn finish_readback(&self, pending: PendingReadback) -> Result<RgbaFrame, RealtimeError> {
        ...
        pending
            .receiver
            .recv()
            .map_err(|_| RealtimeError::Device("readback callback was dropped".to_string()))?
            .map_err(|error| RealtimeError::Device(error.to_string()))?;
    ...
    pub async fn render_at_absolute(
        &mut self,
        session: &RealtimeSession,
        absolute_time_ms: i64,
    ) -> Result<RgbaFrame, RealtimeError> {
        let scene = session.scene_at_absolute(absolute_time_ms)?;
        self.render(&scene).await
    }

    pub async fn render_at_gameplay(
        &mut self,
        session: &RealtimeSession,
        gameplay_time_ms: i64,
    ) -> Result<RgbaFrame, RealtimeError> {
        let scene = session.scene_at_gameplay(gameplay_time_ms)?;
        self.render(&scene).await
    }
    ...
    pub async fn render_stream(
        ...
        callback: impl FnMut(RenderedFrame) -> Result<(), RealtimeError>,
    ) -> Result<(), RealtimeError> {
        self.render_generated_stream(
            requests,
            cancellation,
            |request| session.scene_at_absolute(request.absolute_time_ms),
            callback,
        )
        .await
    }
    ...
    pub(crate) async fn render_generated_stream(
        &mut self,
        requests: impl IntoIterator<Item = FrameRequest>,
        cancellation: &CancellationToken,
        mut scene_at: impl FnMut(FrameRequest) -> Result<FrameScene, RealtimeError>,
        mut callback: impl FnMut(RenderedFrame) -> Result<(), RealtimeError>,
    ) -> Result<(), RealtimeError> {
...
fn normalize_requests(
    requests: impl IntoIterator<Item = FrameRequest>,
) -> Result<Vec<FrameRequest>, RealtimeError> {
    ...
    for request in &requests {
        if !indexes.insert(request.index) {
            return Err(RealtimeError::DuplicateFrameIndex(request.index));
        }
    }
...
async fn select_adapter(
...
) -> Result<wgpu::Adapter, RealtimeError> {
    if let Ok(wanted) = std::env::var("WGPU_ADAPTER_NAME") {
        ...
        return instance
            ...
            .ok_or_else(|| {
                RealtimeError::GpuUnavailable(format!(
    ...
    instance
        ...
        .await
        .map_err(|error| RealtimeError::GpuUnavailable(error.to_string()))
}

fn validate_config(config: OffscreenConfig) -> Result<(), RealtimeError> {
    if config.width == 0
        || config.height == 0
        || config.target_fps == 0
        || config.max_in_flight == 0
    {
        return Err(RealtimeError::InvalidRequest(
...
mod tests {
    ...
    use crate::render::scene::{DrawCommand, SceneRect, SceneSize};
    ...
    fn 流请求在提交前拒绝重复编号并按编号排序() {
        ...
        assert_eq!(duplicate, Err(RealtimeError::DuplicateFrameIndex(2)));
    }
