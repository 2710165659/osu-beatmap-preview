...
use crate::render::scene::{DrawCommand, FrameScene, ResourceId, SceneRect};
...
pub(super) struct SceneRasterizer {
    width: u32,
    height: u32,
    output: wgpu::Texture,
...
impl SceneRasterizer {
    pub(super) fn new(device: &wgpu::Device, width: u32, height: u32, sample_count: u32) -> Self {
    ...
    pub(super) fn output(&self) -> &wgpu::Texture {
        &self.output
    }
...
fn build_batches(
    ...
    target_width: u32,
    target_height: u32,
) -> Result<Vec<DrawBatch>, RealtimeError> {
    let offset = [
    ...
    for command in scene.commands.iter() {
        match command {
            ...
            DrawCommand::PopClip => {
                if clips.len() == 1 {
                    return Err(RealtimeError::Scene(
            ...
            DrawCommand::Sprite {
                ...
                let image = scene.resources.get(resource).ok_or_else(|| {
                    RealtimeError::Scene(format!("frame scene resource {} is missing", resource.0))
                })?;
                push_textured_rect(
                    &mut batches,
            ...
            DrawCommand::Glyph {
                ...
                let image = scene.resources.get(resource).ok_or_else(|| {
                    RealtimeError::Scene(format!("frame scene glyph {} is missing", resource.0))
                })?;
                push_textured_rect(
                    &mut batches,
...
mod tests {
    ...
    use super::*;
    use crate::render::scene::{ResourceId, SceneSize};
