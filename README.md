...
| WASM | `osu-beatmap-preview-wasm-web.zip` | 浏览器 WebGPU 实时渲染，由宿主网页驱动 |

获取方式见 [Releases](https://github.com/2710165659/osu-beatmap-preview/releases)，参数与用法见 [CLI 使用说明](crates/osu-beatmap-preview-cli/README.md)。两类产物都不需要外部 FFmpeg 或额外资源文件。
...
| `osu-beatmap-preview-wasm` | 把 core 会话和 renderer 接到浏览器 WebGPU Canvas（Release 产物） |
| `osu-beatmap-preview-player` | 本地 Web 调试播放器，不属于 Release 产物 |
| `osu-beatmap-preview-gui`、`osu-beatmap-preview-mobile` | 桌面与移动端的 surface、输入、播放生命周期和音频时钟适配骨架 |

CLI 始终走 CPU 导出，不依赖 renderer，也不提供 WGPU 参数；实时 API 在 GPU 不可用时明确报错，不会静默回退到 CPU。crate 之间的边界、配置来源和非目标见[架构说明](docs/architecture.md)。
...
workspace 默认成员只有 CLI，产物为 `target/release/osu-beatmap-preview-cli`（Windows 为 `osu-beatmap-preview-cli.exe`）；WASM 的构建方式见 [WASM 使用说明](crates/osu-beatmap-preview-wasm/README.md)。
