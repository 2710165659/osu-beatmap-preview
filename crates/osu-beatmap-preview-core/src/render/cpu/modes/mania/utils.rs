...
pub fn lane_palette(key_count: i32) -> Vec<Rgba> {
    use super::constants::style::{
        MANIA_COLOR_B, MANIA_COLOR_G, MANIA_COLOR_R, MANIA_COLOR_W, MANIA_COLOR_Y,
    };
...
pub fn darken(color: Rgba, ratio: f64) -> Rgba {
    [
...
pub fn resolve_key_count(beatmap: &Beatmap) -> Result<i32, PreviewError> {
    let cs = beatmap
        .difficulty
        .get_f64("CircleSize")
...
pub fn mania_objects(beatmap: &Beatmap) -> Vec<ManiaHitObject> {
    beatmap
        .hit_objects
        .as_mania()
        .map(|v| v.to_vec())
...
pub fn is_native_mania(beatmap: &Beatmap) -> bool {
...
pub fn beat_length_at(time: i64, timing_points: &[TimingPoint]) -> f64 {
...
pub fn apply_hold_off_mod(hit_objects: &[ManiaHitObject]) -> Vec<ManiaHitObject> {
...
pub fn build_sv_changes(timing_points: &[TimingPoint], chart_end_time: i64) -> Vec<(i64, f64)> {
...
pub fn format_sv_label(sv: f64) -> String {
