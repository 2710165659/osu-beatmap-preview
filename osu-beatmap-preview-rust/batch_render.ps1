# osu-beatmap-preview (Rust) 批量渲染脚本
...
# ── 谱面列表 ──
$std   = @("713703", "2875069", "4077740", "2304032", "3377168")
$taiko = @("4537563", "4700499", "5237055", "4289396", "4507630")
$catch = @("1316292", "1655664", "3169455", "2502343", "944502")
$mania = @("3486603", "256725", "3955355", "4883895", "2610859", "2422376", "1698512", "4972672")

# 构造一个任务对象
function New-Task {
    param($mode, $bid, $fmt, $convert = $null, $mods = $null, $time = $null)
    [pscustomobject]@{
        mode = $mode; bid = $bid; fmt = $fmt
        convert = $convert; mods = $mods; time = $time
    }
}
...
foreach ($t in $tasks) {
    ...
    $psi.FileName = $bin
    $psi.Arguments = ($argList -join " ")   # 用 .Arguments（字符串），兼容 PS 5.1 / 7
...
$lines.Add("osu-beatmap-preview (Rust) 批量渲染报告")
$lines.Add("生成时间: $now")
