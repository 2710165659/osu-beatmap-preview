...
fn build_blob_sprite(base_diameter: f64, layers: &[BlobLayer], seed: u64) -> Img {
    ...
    for layer in layers {
        ...
        let x0 = ((center - half).floor() as i64).max(0);
        let x1 = ((center + half).ceil() as i64).min(sprite_size as i64 - 1);
        for y in x0..=x1 {
            for x in x0..=x1 {
                // 像素在该层的归一化坐标
...
/// 加色混合的水平胶囊（接手判定条）。
fn additive_capsule(
    image: &mut Img, cx: f64, cy: f64, width: f64, height: f64,
    color: [u8; 3], alpha: f64,
) {
    let hw = width / 2.0;
    let hh = height / 2.0;
    let left = cx - hw;
    ...
    let size_w = ((width.ceil() as i64) + 4).max(1) as u32;
    let size_h = ((height.ceil() as i64) + 4).max(1) as u32;
    let ox = (left - 2.0).floor();
    let oy = (top - 2.0).floor();
    let mut scratch = Img::new(size_w, size_h, [0, 0, 0, 0]);

    for y in 0..size_h {
        for x in 0..size_w {
            let px = ox + x as f64 + 0.5;
            let py = oy + y as f64 + 0.5;

            let inside_x = if px < left {
                let d = left - px;
                if d > hh { continue; }
                (1.0 - (d / hh)).max(0.0)
            } else if px > right {
            ...
            let cov = (inside_x * inside_y).clamp(0.0, 1.0);
            if cov > 0.0 {
                let a = (alpha * cov * 255.0).round().clamp(0.0, 255.0) as u8;
                let idx = (y * size_w + x) as usize * 4;
                scratch.data[idx] = color[0];
                scratch.data[idx + 1] = color[1];
                scratch.data[idx + 2] = color[2];
                scratch.data[idx + 3] = a;
            }
        }
    }
    image.additive_composite(&scratch, ox as i64, oy as i64);
}

/// Argon 接手：白色加色胶囊判定条 + 两侧短脚。
pub(crate) fn draw_argon_catcher(
    image: &mut Img, center_x: f64, line_y: f64, capsule_width: f64, scale: f64,
) {
    const WHITE: [u8; 3] = [255, 255, 255];
    let capsule_h = 6.0 * scale;
    let plate_w = capsule_width * 0.8;
    let plate_hw = plate_w / 2.0;
    let plate_h = 3.0 * scale;
    let plate_y = line_y + capsule_h;

    additive_capsule(image, center_x, line_y, capsule_width, capsule_h, WHITE, 1.0);
    for x in [center_x - plate_hw, center_x + plate_hw] {
        image.fill_rect(
            (x - 1.0).round() as i64,
            plate_y.round() as i64,
            (x + 1.0).round() as i64,
            (plate_y + plate_h).round() as i64,
            [255, 255, 255, 200],
        );
    }
}
